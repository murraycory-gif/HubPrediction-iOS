...
describe('host desk-state Soft FAIL wipe after update', () => {
  it('keeps Live cash ON after localStorage wipe — master Live stays OFF', async () => {
    ...
    expect(again.tapes.btc.contracts).toBe(20)
    expect(again.tapes.btc.through).toBe(15)
    expect(again.tapes.btc.centLo).toBe(45)
    expect(again).not.toHaveProperty('liveBets')
    expect(GOLD_RECIPES.btc.liveOn).toBe(true)
    const kept = pickNewerSettings(again, hydrateSettings(null))
    expect((kept as { tapes: { btc: { liveOn: boolean; contracts: number } } }).tapes.btc.liveOn).toBe(true)
    expect((kept as { tapes: { btc: { liveOn: boolean; contracts: number } } }).tapes.btc.contracts).toBe(20)
  ...
  it('finance write cannot restore stale Live cash OFF over a newer pick', async () => {
    ...
    process.env.HUB_DESK_STATE_FILE = join(dir, 'desk-state.json')
    const off = patchTape(loadSettings(), 'btc', { liveOn: false })
