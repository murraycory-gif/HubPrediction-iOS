...
describe('QC2 wires', () => {
  it('quotes poll + send require an OPEN tab — Soft FAIL hidden-tab-only', async () => {
    ...
    expect(dash).toMatch(/acceptAnalystRecipe/)
    ...
    expect(analystSrc).toMatch(/proposed rules — Accept to apply/)
    ...
    expect(financeSrc).toMatch(/export const CLOCK_MAX_SPEND = 25/)
    ...
    expect(dash).toMatch(/stripDeskRows/)
