...
describe('QC0 defaults Soft FAIL Live / live-cash ON', () => {
  it('defaults bake Live print desks ON — Soft FAIL factory 1/off wipe', () => {
    ...
    expect(GOLD_RECIPES.btc).toMatchObject({ liveOn: true, contracts: 30, through: 40, centLo: 69, armFromMin: 8 })
    expect(GOLD_RECIPES.ng).toMatchObject({ liveOn: false, contracts: 30, through: 0.002, armFromMin: 8 })
    expect(GOLD_RECIPES.cu).toMatchObject({ liveOn: false, contracts: 30, through: 0.002, armFromMin: 9 })
    ...
    expect(cashGates(s, 'ng').ok).toBe(false)
    expect(cashGates(s, 'cu').ok).toBe(false)
...
describe('QC2 wires', () => {
  it('quotes poll + send require an OPEN tab — Soft FAIL hidden-tab-only', async () => {
    ...
    expect(tick).toMatch(/export const FEED_RECONNECT_MS = 15_000/)
    expect(tick).not.toMatch(/export const FEED_STALE_MS = 2_500/)
    expect(tick).not.toMatch(/export const FEED_STALE_MS = 8_000/)
    const tapesSrc = await readFile(new URL('../src/lib/tapes.ts', import.meta.url), 'utf8')
    expect(tapesSrc).toMatch(/export const LIVE_PRINT_MS = 250/)
    expect(tapesSrc).toMatch(/export const WARM_PRINT_MS = 250/)
    ...
    expect(data).toMatch(/peekLivePrints/)
    expect(data).toMatch(/startWarm/)
    ...
    expect(analystSrc).toMatch(/proposed rules only — drafts stay paper/)
    ...
    expect(dash).toMatch(/askAllowedByGold\(id, ask, \{ recipe \}\)/)
    const placeSrc = await readFile(new URL('../src/lib/btc-data.ts', import.meta.url), 'utf8')
    expect(placeSrc).toMatch(/askAllowedByGold\(data\.tape, ask, \{ recipe \}\)/)
    ...
    expect(dash).toMatch(/feed-reconnect/)
    expect(dash).toMatch(/feedNeedsReconnect/)
    expect(dash).toMatch(/lastPrintOkAt/)
    expect(dash).toMatch(/reconnecting/)
    expect(dash).not.toMatch(/FEED STALE/)
    ...
    expect(dash).toMatch(/no Live sell/)
    ...
    expect(exitSrc).toMatch(/first down tick sits/)
    expect(exitSrc).toMatch(/already through beat/)
    expect(exitSrc).toMatch(/salvage sits above/)
