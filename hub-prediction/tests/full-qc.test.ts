...
describe('QC2 wires', () => {
  it('quotes poll + send require an OPEN tab — Soft FAIL hidden-tab-only', async () => {
    ...
    expect(dash).toMatch(/tradingActive=\{shownQuote\?\.tradingActive\}/)
    expect(dash).toMatch(/stale=\{stale\}/)
    expect(dash).toMatch(/tradingLive = Boolean\(liveOn && !stale\)/)
    ...
    expect(dash).toMatch(/aria-disabled=\{rehabPaper/)
